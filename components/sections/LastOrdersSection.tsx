...
const LastOrdersSection = () => {
	const { searchTerm, setSearchTerm, filterUsers } = useStateCtx();
	...
	return (
		...
		>
			...
			<div className="hidden sm:flex flex-nowrap items-center justify-between  w-full border-b border-[#edf2f6] dark:border-success/20 text-[#9ca4ab]  dark:text-gray-300  pb-3 ">
				...
				<span className="text-[#9ca4ab]  dark:text-gray-100  font-medium w-full max-w-[100px]  flex items-center   h-full">
					Status
				</span>
				<span className="text-[#9ca4ab]  dark:text-gray-100  font-medium w-full max-w-[100px]  flex items-center   h-full">
					Invoice
			...
			<div className="hidden sm:flex w-full h-[420px] hide-scroll overflow-y-auto flex-col  -mt-[14px]">
				{filterUsers.slice(0, usersNum).map((data, index) => (
					...
					>
						...
						<span className="text-sm min-[700px]:textbase  font-medium w-full max-w-[100px] text-main-sec  dark:text-gray-100  flex  gap-x-2 items-center  h-full">
